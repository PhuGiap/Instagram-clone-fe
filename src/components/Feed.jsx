...
const Feed = () => {
  ...
  const likesByPostId = useSelector((state) => state.like.likesByPostId)
  ...
  return (
    <div className="max-w-md mx-auto p-4 pb-[60px]">
      {allPosts.length === 0 ? (
        ...
        allPosts.map((post) => {
          const liked = likesByPostId[post.id]?.includes(currentUser?.id)
          const likeCount = likesByPostId[post.id]?.length || 0
